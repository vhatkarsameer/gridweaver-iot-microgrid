...
export default function PowerFlow({ onRegionSelect }) {
  ...
  return <section className="dashboard-panel dashboard-flow-panel" aria-label="Regional power flow">
    <div className="dashboard-panel-heading"><div><h2>Regional power flow</h2><p>Distributed grid topology · 5 active regions</p></div><span className="dashboard-live-label"><i /> STREAMING</span></div>
    <div className="dashboard-map-wrap">
      <div className="dashboard-map-status"><i /> LIVE POWER FLOW <span>Updated just now</span></div>
      <svg className="dashboard-flow-map" viewBox="0 0 550 350" role="img" aria-label="Animated regional grid power flow">
        ...
        {NODES.map((node) => <g className={`dashboard-grid-node ${selectedId === node.id ? "selected" : ""}`} key={node.id} transform={`translate(${node.x} ${node.y})`} onClick={() => selectNode(node)}><circle r="18" fill="#0d1b2c" stroke={node.color} strokeWidth="2" /><circle r="5" fill={node.color} /><text y="34" textAnchor="middle">{node.name}</text><text y="49" textAnchor="middle" className="value">{node.value}</text></g>)}
      </svg>
    </div>
    <div className="dashboard-selected-region"><span style={{ background: selected.color }} /><div><small>SELECTED REGION</small><b>{selected.name} <em>{selected.detail}</em></b></div><strong>{selected.value}</strong><div className="dashboard-health"><small>Health</small><b>98.4%</b></div></div>
  </section>;
}
